...
- Week t: Rank stocks by sentiment_value into quintiles within stocks that have sentiment
- Q5 = most positive sentiment, Q1 = most negative sentiment
...
def load_weekly_sentiment() -> pd.DataFrame:
	"""Load sentiment data, aggregate to weekly mean sentiment_value per ticker"""
...
def main() -> None:
	...
	for i, date_t in enumerate(all_dates[:-1]):
		...
		# momentum   (beta > 0): long Q5 (top bullish), short Q1 (top bearish)
		# contrarian (beta < 0): long Q1 (top bearish), short Q5 (top bullish)
		if momentum:
