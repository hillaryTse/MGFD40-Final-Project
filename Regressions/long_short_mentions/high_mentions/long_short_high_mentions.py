...
Long : weekly_mentions in top quintile (>= 80th pct) among mentioned stocks (Dummy = 1)
...
# ── 4. Long leg: top quintile (>= 80th pct) of weekly mentions ────────────────
weekly_q80 = mentions.groupby("date")["mentions"].quantile(0.8).rename("q80")
mentions = mentions.merge(weekly_q80, on="date")
long_df = mentions[mentions["mentions"] >= mentions["q80"]][["date", "ticker"]].copy()
long_df["Dummy"] = 1
...
print(f"\nPanel: {len(panel):,} observations | {panel['ticker'].nunique()} tickers | {panel['date'].nunique()} weeks")
...
print("=" * 55)
for label, s in [("Long (top quintile mentions)", long_ret),
...
# ── 9. Save ───────────────────────────────────────────────────────────────────
panel.to_csv(OUT_DIR / "long_short_high_mentions.csv", index=False)
print(f"\nSaved: {OUT_DIR / 'long_short_high_mentions.csv'}")
